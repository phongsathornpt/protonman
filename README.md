...
Low-concurrency scheduling is `auto` by default. Use `/low` to inspect the configured and effective state, or `/low auto|on|off` to change the persisted session override. `auto` currently recommends the scheduler for OpenCode free models, `on` forces the provider-neutral scheduler for any active model/provider, and `off` bypasses it. The compact footer shows `LOW` only when the scheduler is actually effective, including on narrow terminals.
