...
func TestLifecycleToolsEnforceSessionOwnership(t *testing.T) {
	...
	getCall, _ := tool.NewCall("get-b", "subagent", json.RawMessage(`{"agentId":"agent-session-b"}`))
	...
	cancelCall, _ := tool.NewCall("cancel-b", "subagent", json.RawMessage(`{"agentId":"agent-session-b"}`))
...
func TestResumeAgentCannotCrossSessionBoundary(t *testing.T) {
	...
	call, _ := tool.NewCall("resume-b", "subagent", json.RawMessage(`{"agentId":"strength-9"}`))
