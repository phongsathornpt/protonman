...
func TestResumeAgentStartsFreshChild(t *testing.T) {
	...
	call, _ := tool.NewCall("resume-1", "subagent", json.RawMessage(`{"agentId":"strength-2"}`))
...
func TestResumeAgentRejectsCompletedRun(t *testing.T) {
	...
	handler := NewResumeAgent(coord)
	call, _ := tool.NewCall("resume-complete", "subagent", json.RawMessage(`{"agentId":"agility-1"}`))
