...
func TestSaveUserConfigRejectsCorruptExistingFile(t *testing.T) {
	homeDir := t.TempDir()
	configPath := filepath.Join(homeDir, ".protonman", "config.json")
...
func TestAgentLimitsRejectNegativeValues(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			homeDir := t.TempDir()
			writeConfig(t, filepath.Join(homeDir, ".protonman", "config.json"), `{"agent":{"`+test.field+`": -1}}`)
...
func TestAgentSubagentTimeoutConfigRejectsInvalidValues(t *testing.T) {
	for _, body := range []string{
		`{"agent":{"subagent_max_runtime": "nope"}}`,
		`{"agent":{"subagent_wait_timeout": "0s"}}`,
		`{"agent":{"subagent_queue_timeout": "-1s"}}`,
		`{"agent":{"completed_result_ttl": "0s"}}`,
		`{"agent":{"max_live_subagents": 0}}`,
		`{"agent":{"max_retained_subagents": 0}}`,
	} {
		homeDir := t.TempDir()
		workDir := t.TempDir()
		writeConfig(t, filepath.Join(homeDir, ".protonman", "config.json"), body)
