...
func TestSemanticStylesUseThemePalette(t *testing.T) {
	tests := []struct {
		...
		{"todo active", TodoActiveStyle.GetForeground(), ColorFocus},
		{"todo completed", TodoCompletedStyle.GetForeground(), ColorSuccess},
		{"todo selected", TodoSelectedStyle.GetForeground(), ColorTextPrimary},
