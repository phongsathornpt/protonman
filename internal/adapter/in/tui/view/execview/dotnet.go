...
func summarizeDotnetExec(p *Presentation, output string) {
	...
	case "test":
		if m := dotnetTestRE.FindStringSubmatch(output); len(m) == 4 {
			failed, okFailed := atoiExec(m[1])
			passed, okPassed := atoiExec(m[2])
			skipped, okSkipped := atoiExec(m[3])
			if okFailed && okPassed && okSkipped {
	...
	case "build":
		if m := dotnetBuildRE.FindStringSubmatch(output); len(m) == 3 {
			warnings, okWarnings := atoiExec(m[1])
			errs, okErrs := atoiExec(m[2])
			if okWarnings && okErrs {
