...
func TestActivityFromEventProjectsToolSemantics(t *testing.T) {
	...
	cases := []struct {
		...
		{agent.Event{Kind: agent.EventAgentProgress, Profile: agent.ProfileStrength, Call: call("bash", `{"command":"make lint"}`)}, ActivityDefending},
		{agent.Event{Kind: agent.EventAgentProgress, Profile: agent.ProfileStrength, Call: call("bash", `{"command":"npx eslint ."}`)}, ActivityDefending},
		{agent.Event{Kind: agent.EventAgentProgress, Profile: agent.ProfileStrength, Call: call("bash", `{"command":"echo go test ./..."}`)}, ActivityPushing},
		{agent.Event{Kind: agent.EventAgentProgress, Profile: agent.ProfileStrength, Call: call("bash", `{"command":"git status"}`)}, ActivityPushing},
...
func TestLegendCompactCoversKeyIntents(t *testing.T) {
