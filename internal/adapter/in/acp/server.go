...
func WithMCPRegistryConfigurer(configurer MCPRegistryConfigurer) Option { return func(s *Server) { s.mcpRegistryConfigurer = configurer } }
...
func New(service *toolcall.Service, registry tool.Registry, runner app.Conversation, opts ...Option) (*Server, error) {
	if service == nil { return nil, fmt.Errorf("%w: service is required", ErrInvalidServer) }
	if registry == nil { return nil, fmt.Errorf("%w: registry is required", ErrInvalidServer) }
	s := &Server{service: service, registry: registry, sessions: make(map[string]*Session)}
	for _, opt := range opts { if opt != nil { opt(s) } }
	if s.runnerFactory == nil && runner != nil {
		s.runnerFactory = func(service *toolcall.Service) (app.Conversation, error) { return app.CloneConversationWithTools(runner, service) }
		if _, err := s.runnerFactory(service); err != nil { return nil, fmt.Errorf("%w: runner factory is required for a configured custom runner: %v", ErrInvalidServer, err) }
	}
	return s, nil
}

func (s *Server) Serve(ctx context.Context, input io.Reader, output io.Writer) error {
	if err := ctx.Err(); err != nil { return fmt.Errorf("start ACP server: %w", err) }
	if input == nil || output == nil { return fmt.Errorf("%w: input and output are required", ErrInvalidServer) }
	...
	for scanner.Scan() {
		...
		if parseErr != nil {
			if err := WriteJSON(output, &s.writeMu, parseErr); err != nil { return err }
			continue
		}
		if request.Method != "session/prompt" {
			if err := s.handleRequest(ctx, request, output); err != nil { return err }
			continue
		}
		var params SessionPromptParams
		if err := json.Unmarshal(request.Params, &params); err != nil {
			if err := s.writeResponse(output, request.ID, nil, nil, fmt.Errorf("%w: %v", ErrInvalidRequest, err)); err != nil { return err }
			continue
		}
		params.SessionID = strings.TrimSpace(params.SessionID)
		if params.SessionID == "" {
			if err := s.writeResponse(output, request.ID, nil, nil, errors.New("sessionId is required")); err != nil { return err }
			continue
		}
		sess, ok := s.lookupSession(params.SessionID)
		if !ok {
			if err := s.writeResponse(output, request.ID, nil, nil, fmt.Errorf("unknown session %q", params.SessionID)); err != nil { return err }
			continue
		}
		prompts.Add(1)
		go func(req RPCRequest, sess *Session, blocks []ContentBlock) {
			...
			if err := s.writeResponse(output, req.ID, result, nil, promptErr); err != nil {
				select { case asyncErrors <- err: default: }
			}
	...
	prompts.Wait()
	if err := ctx.Err(); err != nil { return err }
	select { case err := <-asyncErrors: return err; default: }
	if scanErr != nil { return fmt.Errorf("read ACP input: %w", scanErr) }
	return nil
}
...
func watchInputCancellation(ctx context.Context, input io.Reader) func() {
	...
	go func() { select { case <-ctx.Done(): _ = closer.Close(); case <-done: } }()
	return func() { close(done) }
}

func decodeRequest(line []byte) (RPCRequest, *RPCResponse) {
	var request RPCRequest
	if err := json.Unmarshal(line, &request); err != nil { return RPCRequest{}, &RPCResponse{JSONRPC: "2.0", Error: &RPCError{Code: CodeParseError, Message: "parse error"}} }
	if request.Method == "" { return RPCRequest{}, &RPCResponse{JSONRPC: "2.0", ID: request.ID, Error: &RPCError{Code: CodeInvalidRequest, Message: "invalid request: method required"}} }
	return request, nil
}
...
func (s *Server) writeResponse(output io.Writer, id json.RawMessage, result any, notify *RPCNotification, requestErr error) error {
	if requestErr != nil {
		if len(id) == 0 || string(id) == "null" { return nil }
	...
	if notify != nil { if err := WriteJSON(output, &s.writeMu, notify); err != nil { return err } }
	if len(id) == 0 || string(id) == "null" { return nil }
...
func (s *Server) dispatch(ctx context.Context, request RPCRequest, output io.Writer) (any, *RPCNotification, error) {
	...
	case "session/new":
		...
		if len(request.Params) > 0 { if err := json.Unmarshal(request.Params, &params); err != nil { return nil, nil, fmt.Errorf("decode session/new: %w", err) } }
		if err := validateMCPServerConfigs(params.MCPServers); err != nil { return nil, nil, fmt.Errorf("session/new MCP servers: %w", err) }
		sessionID := session.NewID(params.Cwd)
		sess, err := s.newSession(ctx, sessionID, params.Cwd, params.MCPServers)
		if err != nil { return nil, nil, err }
		s.mu.Lock(); s.sessions[sessionID] = sess; s.mu.Unlock()
	...
	case "session/load":
		...
		if err := json.Unmarshal(request.Params, &params); err != nil { return nil, nil, fmt.Errorf("decode session/load: %w", err) }
		params.SessionID = strings.TrimSpace(params.SessionID)
		if params.SessionID == "" { return nil, nil, errors.New("sessionId is required") }
		if err := validateMCPServerConfigs(params.MCPServers); err != nil { return nil, nil, fmt.Errorf("session/load MCP servers: %w", err) }
		sess, err := s.loadOrCreateSession(ctx, params.SessionID, params.Cwd, params.MCPServers)
		if err != nil { return nil, nil, err }
		if err := sess.ReplayHistory(func(notification RPCNotification) error { return WriteJSON(output, &s.writeMu, notification) }); err != nil { return nil, nil, err }
		return nil, nil, nil
	case "session/resume":
		var params SessionResumeParams
		if err := json.Unmarshal(request.Params, &params); err != nil { return nil, nil, fmt.Errorf("decode session/resume: %w", err) }
		params.SessionID = strings.TrimSpace(params.SessionID)
		if params.SessionID == "" { return nil, nil, errors.New("sessionId is required") }
		if err := validateMCPServerConfigs(params.MCPServers); err != nil { return nil, nil, fmt.Errorf("session/resume MCP servers: %w", err) }
		_, err := s.loadOrCreateSession(ctx, params.SessionID, params.Cwd, params.MCPServers)
		return nil, nil, err
	case "session/set_mode":
		var params SessionSetModeParams
		if err := json.Unmarshal(request.Params, &params); err != nil { return nil, nil, fmt.Errorf("decode session/set_mode: %w", err) }
		params.SessionID = strings.TrimSpace(params.SessionID)
		if params.SessionID == "" { return nil, nil, errors.New("sessionId is required") }
		sess, ok := s.lookupSession(params.SessionID)
		if !ok { return nil, nil, fmt.Errorf("unknown session %q", params.SessionID) }
		mode, err := permission.ParseMode(params.ModeID)
		if err != nil { return nil, nil, fmt.Errorf("invalid mode %q: %w", params.ModeID, err) }
		if err := sess.service.SetMode(mode); err != nil { return nil, nil, fmt.Errorf("set session mode: %w", err) }
		if err := sess.saveStateDetached(ctx); err != nil { return nil, nil, fmt.Errorf("save session %q: %w", params.SessionID, err) }
	...
	case "session/cancel":
		...
		if err := json.Unmarshal(request.Params, &params); err != nil { return nil, nil, fmt.Errorf("decode session/cancel: %w", err) }
		params.SessionID = strings.TrimSpace(params.SessionID)
		if params.SessionID == "" { return nil, nil, errors.New("sessionId is required") }
		sess, ok := s.lookupSession(params.SessionID)
		if !ok { return nil, nil, fmt.Errorf("unknown session %q", params.SessionID) }
	...
	case "session/list":
		...
		if len(request.Params) > 0 { if err := json.Unmarshal(request.Params, &params); err != nil { return nil, nil, fmt.Errorf("decode session/list: %w", err) } }
	...
	case "session/delete":
		...
		if err := json.Unmarshal(request.Params, &params); err != nil { return nil, nil, fmt.Errorf("decode session/delete: %w", err) }
		params.SessionID = strings.TrimSpace(params.SessionID)
		if params.SessionID == "" { return nil, nil, errors.New("sessionId is required") }
		if err := s.deleteSession(ctx, params.SessionID); err != nil { return nil, nil, err }
		return nil, nil, nil
	default:
...
func (s *Server) loadOrCreateSession(ctx context.Context, sessionID string, cwd string, mcpServers []MCPServerConfig) (*Session, error) {
	s.mu.Lock(); existing, ok := s.sessions[sessionID]; s.mu.Unlock()
	if ok { if err := existing.matchMCPServers(mcpServers); err != nil { return nil, err }; return existing, nil }
	sess, err := s.newSession(ctx, sessionID, cwd, mcpServers)
	if err != nil { return nil, err }
	if s.sessionService != nil {
		state, found, err := s.sessionService.Load(ctx, sessionID)
		if err != nil { return nil, fmt.Errorf("load session state %q: %w", sessionID, err) }
		if found {
			if cwd != "" && state.WorkspaceKey != "" && state.WorkspaceKey != session.WorkspaceKey(cwd) { return nil, fmt.Errorf("session %q belongs to another workspace", sessionID) }
			if state.WorkspaceKey != "" { sess.workspaceKey = state.WorkspaceKey }
			if state.WorkspaceName != "" { sess.workspaceName = state.WorkspaceName }
			...
			if err != nil { return nil, fmt.Errorf("session permission mode %q: %w", sessionID, err) }
			if err := sess.service.SetMode(mode); err != nil { return nil, fmt.Errorf("restore session mode %q: %w", sessionID, err) }
			if strings.TrimSpace(state.ReasoningEffort) != "" {
				effort, parseErr := sdk.ParseReasoningEffort(state.ReasoningEffort)
				if parseErr != nil { return nil, fmt.Errorf("restore session reasoning %q: %w", sessionID, parseErr) }
				if err := sess.SetReasoningEffort(effort); err != nil { return nil, fmt.Errorf("restore session reasoning %q: %w", sessionID, err) }
			}
		}
	}
	s.mu.Lock(); s.sessions[sessionID] = sess; s.mu.Unlock()
...
func (s *Server) newSession(ctx context.Context, sessionID string, cwd string, mcpServers []MCPServerConfig) (*Session, error) {
	...
	if s.sessionRegistryFactory != nil {
		...
		if err != nil { return nil, fmt.Errorf("create registry for session %q: %w", sessionID, err) }
		registry = created
	}
	if len(mcpServers) > 0 {
		if s.mcpRegistryConfigurer == nil { return nil, fmt.Errorf("configure MCP servers for session %q: MCP server configuration is not available", sessionID) }
		resource, err := s.mcpRegistryConfigurer(ctx, cwd, registry, cloneMCPServerConfigs(mcpServers))
		if err != nil { return nil, fmt.Errorf("configure MCP servers for session %q: %w", sessionID, err) }
		mcpResource = resource
	}
	service, err := s.service.CloneWithRegistry(registry)
	if err != nil { if mcpResource != nil { _ = mcpResource.Close() }; return nil, fmt.Errorf("%w: clone session tool-call service: %v", ErrInvalidServer, err) }
	var runner app.Conversation
	if s.runnerFactory != nil {
		created, err := s.runnerFactory(service)
		if err != nil { if mcpResource != nil { _ = mcpResource.Close() }; return nil, fmt.Errorf("create runner for session %q: %w", sessionID, err) }
		runner = created
	}
...
func (s *Server) listSessions(ctx context.Context, cwd string) ([]SessionInfo, error) {
	...
	for id, sess := range s.sessions {
		if cwd != "" && sess.cwd != "" && sess.cwd != cwd { continue }
		seen[id] = true
	...
	if s.sessionService != nil {
		...
		if cwd != "" { options.WorkspaceKey = session.WorkspaceKey(cwd) }
		summaries, err := s.sessionService.ListSummaries(ctx, options)
		if err != nil { return nil, fmt.Errorf("list session state: %w", err) }
		for _, summary := range summaries {
			if seen[summary.ID] { continue }
			title := "Session " + summary.ID
			if summary.WorkspaceName != "" { title = summary.WorkspaceName + " · " + summary.ID }
...
func (s *Server) deleteSession(ctx context.Context, sessionID string) error {
	s.mu.Lock(); sess := s.sessions[sessionID]; s.mu.Unlock()
	if sess != nil { sess.Cancel() }
	if _, err := s.agents.ForSession(sessionID).CancelSessionAndWait(ctx); err != nil { return fmt.Errorf("cancel session subagents %q: %w", sessionID, err) }
	if s.sessionService != nil { if err := s.sessionService.Delete(ctx, sessionID); err != nil { return fmt.Errorf("delete session state %q: %w", sessionID, err) } }
	s.mu.Lock(); sess = s.sessions[sessionID]; delete(s.sessions, sessionID); s.mu.Unlock()
	if sess != nil { if err := sess.Close(); err != nil { return fmt.Errorf("close session %q: %w", sessionID, err) } }
	return nil
}
...
func (s *Server) closeSessions() {
	...
	for _, sess := range s.sessions { sessions = append(sessions, sess) }
	s.mu.Unlock()
	for _, sess := range sessions { _ = sess.Close() }
}
