...
func nextLifecycleEvent(status AgentStatus, kind LifecycleEventKind, at time.Time, reason string) LifecycleEvent {
	return LifecycleEvent{
		...
		Profile: status.Profile, Task: status.Task, Optional: status.Optional, Provider: status.Provider, Model: status.Model,
...
func applyLifecycleEvent(status AgentStatus, event LifecycleEvent) (AgentStatus, error) {
	...
	if status.State == "" {
		...
		status = AgentStatus{
			...
			Profile: event.Profile, Provider: event.Provider, Model: event.Model, Task: event.Task, Optional: event.Optional,
