...
func TestFromModelMessagesCanonicalizesLegacyToolNames(t *testing.T) {
	messages := []sdk.Message{
		{Role: sdk.RoleAssistant, ToolCalls: []sdk.ToolCall{{ID: "call-1", Name: "read_file"}}},
		{Role: sdk.RoleTool, ToolCallID: "call-1", ToolName: "read_file", Content: "legacy output"},
	}
	persisted := FromModelMessages(messages)
	if len(persisted) != 1 {
		t.Fatalf("persisted messages = %#v", persisted)
	}
	if strings.Contains(persisted[0].Content, "read_file") {
