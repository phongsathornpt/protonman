...
type CostEstimator func([]sdk.Message) (int, error)
...
// normal retention.
func RetainByCost(messages []sdk.Message, recentMessages, maxHistoricalToolResultBytes, budget int, estimator CostEstimator) ([]sdk.Message, error) {
	if len(messages) == 0 {
		return nil, nil
	}
	if estimator == nil {
	...
	if len(spans) == 0 {
		return append([]sdk.Message(nil), messages...), nil
	}
...
func costRetentionCandidate(messages []sdk.Message, leadingSystems, start int) []sdk.Message {
