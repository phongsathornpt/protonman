...
func toolDisciplineSection(spec Spec) string {
	lines := []string{
		...
		"- Prefer dedicated workspace tools over shell equivalents: read_file for exact file reads, grep for simple content search, find_files for recursive path discovery, inspect_code for multi-file search plus contextual source inspection, list_dir for directory listing, and write_file/search_replace/apply_patch for file changes.",
		"- Use bash for executing programs, builds, tests, package managers, and shell behavior that dedicated tools cannot express; do not use Python, Node, Ruby, Perl, awk, cat/head/tail/sed/nl/grep/rg/find/ls/echo/printf, shell redirection, or ad-hoc scripts to discover, search, scan, or read workspace source when inspect_code, find_files, grep, list_dir, or read_file can perform the task.",
