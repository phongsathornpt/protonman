...
func TestWaitAgentTimeoutDoesNotCancelChild(t *testing.T) {
	...
	h, err := coord.Spawn(context.Background(), agent.Request{Profile: agent.ProfileExplorer, Task: "background"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestAgentLifecycleGetListCancel(t *testing.T) {
	...
	defer coord.Close()
	h, err := coord.Spawn(context.Background(), agent.Request{Profile: agent.ProfileExplorer, Task: "cancel target"})
	if err != nil {
		t.Fatal(err)
	}
