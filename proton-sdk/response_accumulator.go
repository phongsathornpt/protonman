...
// normalized stream events. It mirrors the response-accumulator boundary used
// by proton-sdk-rs while retaining the Go SDK's existing event surface.
...
func (a *ResponseAccumulator) Absorb(event Event) error {
	...
	case EventUsage:
		// Usage events are currently normalized as complete snapshots by the Go
		// provider adapters. Keep the latest snapshot until the usage model gains
		// explicit optional/partial fields like proton-sdk-rs.
