...
func TestRetryZeroPolicyUsesCanonicalDefaults(t *testing.T) {
	if DefaultRetryBaseBackoff != 5*time.Second {
		t.Fatalf("DefaultRetryBaseBackoff = %v", DefaultRetryBaseBackoff)
	}
	if DefaultRetryMaxBackoff != 60*time.Second {
		t.Fatalf("DefaultRetryMaxBackoff = %v", DefaultRetryMaxBackoff)
	}
	if DefaultRetryMaxAfter != 30*time.Second {
		t.Fatalf("DefaultRetryMaxAfter = %v", DefaultRetryMaxAfter)
	}
	if got := RetryDelay(1, RetryPolicy{}); got != DefaultRetryBaseBackoff {
		t.Fatalf("RetryDelay(1, zero) = %v, want %v", got, DefaultRetryBaseBackoff)
	}
	want := []time.Duration{5 * time.Second, 15 * time.Second, 30 * time.Second, 60 * time.Second}
	for retry, expected := range want {
		if got := RetryDelay(retry+1, RetryPolicy{}); got != expected {
