...
- `model/`: provider presets, discovery, catalog normalization, SDK adaptation, and narrowly scoped provider-specific wrappers. OpenCode models can use a shared endpoint+model low-concurrency scheduler here for bounded admission, low concurrency, and adaptive pacing. The default `auto` policy enables it for OpenCode free models; the TUI `/low` command can force it on or off for the active session. Replay-safe free-model stream recovery remains a separate wrapper, and provider-neutral SDK behavior stays outside this policy.
