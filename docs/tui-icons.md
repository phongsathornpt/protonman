...
`nerd` uses the curated Nerd Fonts v3 icon set in `internal/adapter/in/tui/view/icon`. Protonman supports the **Nerd Font Mono** terminal variant for this mode so icon prefixes retain the existing one-cell glyph plus one trailing-space layout contract.
...
The process running Protonman cannot reliably determine which font the terminal emulator is using. This is especially important over SSH, where Protonman may run on a remote host while glyph rendering happens in the local terminal.
...
Raw Nerd Font private-use codepoints belong only in the icon presentation package. Renderers consume semantic fields such as `Icons.Read`, `Icons.Edit`, `Icons.Agent`, and `Icons.ToolSuccess`.
...
Raw/copyable transcript output remains text or portable Unicode. Nerd Font glyphs are presentation-only and should not become part of persisted conversation data, tool results, or execution policy.
...
The Nerd profile is pinned to Nerd Fonts v3 codepoints and intentionally contains only the small semantic subset Protonman renders. Do not copy arbitrary legacy codepoints into individual views.
